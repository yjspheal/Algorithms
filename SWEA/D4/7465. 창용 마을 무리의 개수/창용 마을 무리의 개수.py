...
def dfs(start_p, adj_list, check_group):
    ...
    Args:
        start_p : 시작 사람
        adj_list : 인접리스트
        check_group : 무리 생성 여부가 담긴 boolean 리스트
    """
    ...
    while stack:
        ...
        if not check_group[curr_p]:       # 무리가 없는 사람이면
            check_group[curr_p] = True     # 무리에 넣어준다

            for next_p in adj_list[curr_p]:  # 연결된 사람들도 똑같이 진행한다
